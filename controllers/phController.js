...
class pHController {
    // logph is a method that handles the insertion of pH level data into the database
    // It expects 2 parameters in the request object: req.params.deviceID and req.params.ph
    async logPh(req, res){
        console.log(req.params.ph)
        console.log(req.params.deviceID)
        if(req.params.deviceID != null && req.params.ph != null) {
            let deviceID = req.params.deviceID // Sets the deviceID variable to the deviceID from the parameter attribute of the request
            let ph = req.params.ph; // Sets the ph variable to the ph value of the params attribute of the request
            var sql = `insert into log_ph (log_date, device_id, nivel_ph) values (now(), ${deviceID}, ${ph});`
            // SQL query that updates the log_date, device_id and ph fields of the log_ph table
    ...
    async getPhLogs(req,res){
        ...
        if(req.params.deviceID!=null){
            ...
            var sql = `select * from log_ph where device_id='${deviceID}';` // SQL query string for the retrieval
...
const phController = new pHController() // New instance of the MainController class
// by calling its constructor with the new keyword. This is for the phController
module.exports = phController; // The phController instance of MainController is exported
